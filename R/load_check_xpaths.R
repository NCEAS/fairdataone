...
# Summarize checks
#
# This function grabs a table of all xpaths, the check they correspond to, and
...
summarize_checks <- function() {
    ...
    fair_suite_def <-
        read_xml(paste0(t, "/metadig-checks-master/src/suites/FAIR-suite.xml"))
    ...
    check_names <- fair_suite_def %>%
        ...
        xml_text() %>%
        gsub(".1|.2", "", .) %>%
        gsub("resource.abstractLength.sufficient",
             "resource.abstractLength",
             .)
    ...
    fair_checks <-
        data.frame(check_names, check_type, check_level, stringsAsFactors = F)

    # run get description function over all checks
    fair_checks$description <- map_chr(check_names, get_description, t)

    # extract xpaths for all checks
    xpaths <- purrr::map(check_names, parse_xpath, t)

    fair_checks$xpath <- xpaths
    # expand data frame so each row corresponds to an xpath
    fair_checks_ex <- fair_checks %>%
        unnest(cols = .data$xpath)

    # run function to assign dialect to each xpath
    fair_checks_ex$dialect <-
        map_chr(fair_checks_ex$xpath, assign_dialect)

    return(fair_checks_ex)
}

# define function to read individual check file and get
# check description
get_description <- function(check_name, t) {
...
parse_xpath <- function(check_name, t) {
    base_path <- paste0(t, "/metadig-checks-master/src/checks/")
